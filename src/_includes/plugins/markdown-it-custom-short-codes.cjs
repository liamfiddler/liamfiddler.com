// Based on https://github.com/furutsubaki/markdown-it-custom-short-codes
// (the only change was to update the markerPatter to use single square braces, instead of double)

const markerPattern = /^\[(.*)\]/im;

module.exports = (md, _o) => {
    const cscRegexp = markerPattern;

    function csc(state, startLine, _endLine, silent) {
    ...
    // set render csc
    md.renderer.rules.csc = function (tokens, index, _type) {
